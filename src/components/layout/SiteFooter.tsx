...
export function SiteFooter({ brand, navigation = site.navigation, email = site.email }: { brand?: PublicBrandSettings; navigation?: readonly { label: string; href: string }[]; email?: string }) {
  return <footer className="footer">
    <div className="footer-top"><div><span className="eyebrow">PICVISUAL / GLOBAL</span><p>Image and motion finishing<br />for commercial visual worlds.</p></div><a className="footer-email" href={`mailto:${email}`}>{email} <i>↗</i></a></div>
    <div className="footer-links">{navigation.map((item) => <Link key={item.href} href={item.href}>{item.label}<i>↗</i></Link>)}<Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link></div>
