...
type SessionPayload = { userId: string; role: "OWNER" | "ADMIN" | "EDITOR" };
...
export async function readSessionToken(token: string | undefined): Promise<SessionPayload | null> {
  ...
  try { const { payload } = await jwtVerify(token, getSecret()); return typeof payload.userId === "string" && (payload.role === "OWNER" || payload.role === "ADMIN" || payload.role === "EDITOR") ? { userId: payload.userId, role: payload.role } : null; } catch { return null; }
}
