...
export const createUserSchema = z.object({ name: z.string().trim().max(100).optional(), email: z.string().trim().email().max(255), password: z.string().min(12, "Use at least 12 characters.").max(72), role: z.enum(["OWNER", "ADMIN", "EDITOR"]) });
